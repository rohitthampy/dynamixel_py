...
class ServoGroup:
    ...
    def sync_get_positions(self, is_radian: bool = False):
        ref_servo = self.servos[list(self.servos)[0]] # Taking the first servo's config as reference
        ...
        for dxl_id in self.servos.keys():
            if sync_read.isAvailable(dxl_id, start_addr, data_length):
                ...
                if is_radian:
                    angle = pi * float(raw_position) / self.servos[dxl_id].middle_pos_val
                else:
                    angle = 180 * float(raw_position) / self.servos[dxl_id].middle_pos_val
                current_positions.append(angle)
            else:
                raise RuntimeError(f"group_sync_read failed for servo with id: {dxl_id}")
    ...
    def sync_set_positions(self, goal_positions : list[float], is_radian: bool = False):
        ...
        sync_write = GroupSyncWrite(port=port_h, ph=packet_h, start_address=start_addr, data_length=data_length)

        for i, dxl_id in enumerate(self.servos.keys()):

            if is_radian:
                goal_pos = int(self.servos[dxl_id].middle_pos_val * goal_positions[i] / pi)
            else:
                goal_pos = int(self.servos[dxl_id].middle_pos_val * goal_positions[i] / 180)
